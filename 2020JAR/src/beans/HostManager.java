...
public class HostManager {
		...
		if (!ip.getHostAddress().equals(MASTERIP)) {
			...
			if (handshake(new Host(ip.getHostName(), ip.getHostAddress(), false))) {
				...
				if (handshake(new Host(ip.getHostName(), ip.getHostAddress(), false))) {
					...
					String hostPath = "http://" + MASTERIP + ":8080/2020WAR/rest/server/node/informmaster/" + myAlias + "/" + MASTERIP;
	...
	public boolean handshake(Host newHost) {
		...
		// STEP 2.1
		// Novi cvor salje masteru sve tipove agenata koje ima
		for (AgentType at1 : db.getAgentTypes().values()) {
			try {
				...
				System.out.println("ERROR IN STEP 2.1");
				return false;
			}
		}
		...
		// STEP 4.5
		// Novi cvor pita master za spisak svih tipova agenata i master mu u odgovoru salje
		try {
			ResteasyClient client = new ResteasyClientBuilder().build();
			ResteasyWebTarget target = client.target(PATH+ "agents/classes");
			Response res = target.request(MediaType.APPLICATION_JSON).get();
			AgentType[] ret = res.readEntity(AgentType[].class);
			System.out.println("ALL AGENT TYPES ARE BEING PASSED TO NEW NODE");
			for (AgentType aType : ret) {
				System.out.println("GOTTEN AGENT TYPE: " + aType.getName());
				if (!db.getAgentTypes().containsKey(aType.getName())) {
					System.out.println("NEW AGENT TYPE ADDING: " + aType.getName());
					db.getAgentTypes().put(aType.getName(), aType);
				}
			}
		}
		catch (Exception e) {
			System.out.println("ERROR IN STEP 4.5");
			return false;
		}
		
		
		// STEP 4.7
		// New node asking master for all running agents
		try {
			ResteasyClient client = new ResteasyClientBuilder().build();
			...
			System.out.println("ERROR IN STEP 4.7");
			return false;
		}
